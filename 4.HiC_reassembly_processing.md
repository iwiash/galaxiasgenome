...
not sure if i should round the G= or not....

maybe also run the ark one? idk what that is but i do have a kmer database from meryl so...
```
longstitch tigmint-ntLink-arks draft=gb_hic_purged.fa reads=gbrev_hifi_reads.fq.gz kmer_db=(meryl kmer database) t=8 out_prefix=gb_hic_fulldraft_arks G=3.02465058e8 longmap=hifi
```
