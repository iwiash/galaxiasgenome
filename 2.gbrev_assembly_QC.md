...
```
output pm same as before purging
```
C:99.6%[S:96.1%,D:3.5%],F:0.0%,M:0.4%,n:255	   
 254	Complete BUSCOs (C)			   
 245	Complete and single-copy BUSCOs (S)	   
...
```
samtools view -b GB_aln.sam > GB_aln.bam -@ 4

samtools sort -m 5G -o GB_aln_sorted.bam -T sorting.temp GB_aln.bam
```
...
```
[10-10-2024 17:35:57] Beginning read-depth histogram generation
[10-10-2024 17:35:57] running genome coverage analysis on GB_aln_sorted.bam
[10-10-2024 17:44:54] ERROR: Failed to close pipe: samtools depth -a -r "ptg000002l_1" GB_aln_sorted.bam 2>> tmp_purge_haplotigs/STDERR/samtoolsDepth.stderr | (this script), check tmp_purge_haplotigs/STDERR/samtoolsDepth.stderr
PIPELINE FAILURE
...
```
samtools reheader -c "perl -lne '@F = split(qq[ ], \$_); foreach \$i (@F) { \$i =~ s/^(..)ID:/\$1:/; } print join(qq[\\t], @F);'" GB_aln.bam > GB_aln_reheader.bam
[E::sam_hrecs_error] Malformed key:value pair at line 641: "@PG ID:minimap2     PN:minimap2     VN:2.28-r1209   CL:minimap2     -t      16      -ax     map-hifi        GB_full.p_ctg.fa        gbrev_hifi_reads.fq.gz"
```
