...
issue appears to be with indexing? cant find index file so running index step by itself
```
bwa index -p gb_hic_purged.fa gb_hic_purged.fa
```
hoping this is the correct output to be able to feed back into the snakefile, but i cant seem to find what is actually wrong with the snakefile itself
