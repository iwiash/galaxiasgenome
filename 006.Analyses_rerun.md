...
```
#!/bin/bash -e
#SBATCH --job-name=bayescan_PO_1000 # Job name (shows up in the queue)
#SBATCH --time=9:00:00      # Walltime (HH:MM:SS) 48h
...
GESTE="maf_no_offshore_islands_bayescan_input.geste"

bayescan_2.1 $GESTE \
        -pr-odds 1000 \
        -o bayescan_no_islands_PO_1000
        -threads 16 \
...
[R PLOTTING AND PROCESSING](R_SCRIPTS/bayescan_results_processing.Rmd)
...
[R PLOTTING](R_SCRIPTS/pca_plots.R) AND [PROCESSING](R_SCRIPTS/pca_analyses_all_samples.Rmd)
...
```
[R PLOTTING](R_SCRIPTS/admixture_plots_thesis_ver.R) AND [PROCCESING](R_SCRIPTS/plotting_admixture.Rmd)

### LDna - didn't end up using
...
```
run for all pops then this for all:

```

```

[R PLOTTING](R_SCRIPTS/satirway_allpops_working_md.Rmd)
