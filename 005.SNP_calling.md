...
```
this worked now it moves on to mem step but i haven't fixed in the pipeline yet
...
```
sorts based on row 4 which is the missing snps
...
```
## Remove the snps present in the negative ctrl from all the other samples 
and also rm koaro_chl18
...
```
remove snps that are not present in any of the three samples
```
