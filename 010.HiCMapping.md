...
ln -s /home/iwias275/nobackup/source_files/hic/mapping/juicer/scripts/juicer_tools.1.9.9_jcuda.0.8.jar juicer_tools.jar
cd /home/iwias275/nobackup/source_files/hic/mapping/juicer

mkdir references
cd references/
ln -s ../../hicdata/reordered_gb_hic_purged_masked.fa
mv ../../hicdata/sizes.genome . ## (generated with cut -f1,2 reordered_gb_hic_purged_masked.fa.fai > sizes.genome)

cd ../
mkdir fastq
cd fastq/
ln -s ../../hicdata/TRIMMED_HiC_R1.fastq.g
ln -s ../../hicdata/TRIMMED_HiC_R2.fastq.g
...
Use generate-site-pos to in silico digest reference genome:
```
## first hard-code galaxias genome into py script:
## add to 'filenames'
'galaxias': './references/reordered_gb_hic_purged_masked.fa',
```
```
python juicer/misc/generate_site_positions.py Arima galaxias references/reordered_gb_hic_purged_masked.fa
mv galaxias_Arima.tx restriction_sites/
```
produces 'galaxias_Arima.txt' and move to restriction site dir
